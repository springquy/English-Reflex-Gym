...
export const Game: React.FC<GameProps> = ({ settings, questionsSource, onEnd, onExit }) => {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 overflow-hidden font-sans">
      {/* HEADER: Compact Height h-16 */}
      <header className="px-6 bg-white border-b border-slate-200 flex justify-between items-center z-10 shrink-0 h-16">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                {isTimeUnlimited ? 'Relaxing' : `${settings.timePerQuestion}s Blitz`}
              </span>
            </div>
        ...
        <div className="flex items-center gap-3">
          <div className="bg-slate-900 text-white px-4 py-1.5 rounded-xl font-black text-sm shadow-md shadow-slate-200">
      ...
      {/* MAIN: Adjusted Grid and Sizing for better responsiveness */}
      <main className="flex-1 overflow-y-auto p-4 md:p-6 flex items-center justify-center">
        <div className="w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 items-start md:items-center h-full md:h-auto">
          
          {/* Left Column (Question + Mic) - span 7 */}
          <div className="md:col-span-7 flex flex-col gap-6 md:gap-8">
            ...
            {/* Question Text: Reduced from 7xl to 4xl/5xl for better fit */}
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 leading-[1.1] tracking-tight animate-in fade-in slide-in-from-left-4 duration-500 delay-75">
               {currentQ.vietnamese}
            </h2>

            {/* Mic/Transcript Area: Reduced Min-Height */}
            <div className={`
              min-h-[120px] md:min-h-[160px] w-full p-6 rounded-[2rem] flex flex-col items-center justify-center relative transition-all duration-200 border-2
                ...
                {currentQ.hint?.structure || currentQ.hint?.vocab ? (
                    <>
                        ...
                        <button 
                            ...
                            className="flex items-center gap-2 text-slate-400 hover:text-indigo-600 font-black text-[10px] uppercase tracking-widest transition-all px-6 py-4 rounded-2xl hover:bg-white hover:shadow-md border border-transparent hover:border-slate-100 group"
                        >
                            <HelpCircle className="w-5 h-5 group-hover:scale-110 transition-transform" /> 
                            Gợi ý <span className="ml-1 bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-md text-[9px] border border-slate-200">Q</span>
                        </button>
                        ...
                        >
                            ...
                            <p className="text-lg md:text-xl font-bold text-amber-900 font-mono tracking-tight">{currentQ.hint.structure}</p>
                        ...
                        >
                            ...
                            <p className="text-lg md:text-xl font-bold text-blue-900 font-mono tracking-tight">{currentQ.hint?.vocab || '...'}</p>
            ...
            {gameState === GameState.REVIEWING && feedback && (
              <div className="w-full bg-white rounded-[2rem] p-6 md:p-8 border border-slate-200 shadow-[0_10px_30px_rgba(0,0,0,0.05)] animate-in slide-in-from-right-8 duration-500">
                ...
                <div className="space-y-4">
                  ...
                  {currentQ.note && (
                    <div className="p-4 bg-indigo-50/40 rounded-xl flex gap-3 items-start border border-indigo-100/50">
      ...
      {/* FOOTER: Compact Height h-20 */}
      <footer className="px-6 py-4 bg-white border-t border-slate-100 shrink-0 z-10 h-20 flex items-center justify-center">
        <div className="w-full max-w-md">
          {gameState === GameState.REVIEWING ? (
            <div 
              onClick={nextQuestion}
              className="w-full py-3.5 bg-[#0f172a] text-white rounded-2xl font-black text-lg shadow-lg shadow-slate-200 flex items-center justify-center gap-3 cursor-pointer hover:bg-slate-800 transition-all active:scale-95 group"
            >
             ...
             <div className="w-full py-3.5 bg-slate-50 border-2 border-slate-100 text-slate-400 rounded-2xl font-black text-lg flex items-center justify-center gap-2 cursor-wait">
               <Loader2 className="w-5 h-5 animate-spin" /> ĐANG XỬ LÝ...
            </div>
          ) : isSpacePressed ? (
            <div className="w-full py-3.5 bg-indigo-50 border-2 border-indigo-100 text-indigo-600 rounded-2xl font-black text-lg flex items-center justify-center gap-3 animate-pulse shadow-sm">
               <div className="w-3 h-3 bg-red-500 rounded-full animate-ping"></div>
               THẢ SPACE ĐỂ GỬI
            </div>
          ) : (
             <div className="w-full py-3.5 bg-white border-b-4 border-slate-100 text-slate-400 rounded-2xl font-bold text-lg flex items-center justify-center gap-2">
               Giữ <span className="bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 text-slate-600 font-black text-base">Space</span> để nói
            </div>
          )}
        </div>
      </footer>
